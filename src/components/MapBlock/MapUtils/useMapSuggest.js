...
export function useMapSuggest() {
        ...
        ymapsRef.current
          ...
          .then((res) => {
            ...
            if (firstGeoObject) {
              ...
              const isBigRegion =
                regionText.includes("область") ||
                regionText.includes("край") ||
                regionText.includes("Республика");
  ...
  useEffect(() => {
    return () => {
      if (
        suggestRef.current &&
        typeof suggestRef.current.destroy === "function"
      ) {
